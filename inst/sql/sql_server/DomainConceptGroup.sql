...
SELECT DISTINCT descendant_concept_id,
  ancestor_concept_id
INTO #groups
FROM @cdm_database_schema.concept_ancestor INNER JOIN (
												select concept_id
												FROM @cdm_database_schema.concept
												INNER JOIN (
															SELECT *
															FROM @cdm_database_schema.concept_ancestor
															WHERE DESCENDANT_CONCEPT_ID IN (4293902,4067285,4010109,4238814,4238720,4075540,4158892,4300372,4034458,4309438,4203936,4296695,4275564,4145752,4034680,4194253,4323283,2000283,4301352)
															) temp
															 ON concept_id = descendant_concept_id
